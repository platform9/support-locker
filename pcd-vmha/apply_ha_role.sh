...
usage() {
    ...
    echo "       $0 --help"
    echo ""
    echo "Reads a host list file (generated by list_missing_ha_hosts.sh) and applies"
    echo "the 'pf9-ha-slave' role to eligible hosts via PUT API calls."
    echo ""
    echo "Arguments:"
    echo "  <hosts_file>  File generated by list_missing_ha_hosts.sh (missing_hosts_<timestamp>.txt)"
    echo "                Format per line: host_id | hostname | cluster_name | responding | pf9-ostackhost-neutron_status | current_roles"
    echo ""
    echo "Options:"
    echo "  --help, -h    Show this help message and exit"
    echo ""
    echo "Eligibility: Only hosts with responding=true AND pf9-ostackhost-neutron=applied are processed."
...
# ─── Handle --help and validate arguments ───────────────────────────────────
